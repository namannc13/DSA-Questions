...
public class partitionEqualSubsetSum {
    public static void main(String[] args) {
        ...
        System.out.println(partitionEqualSubsetSumOptimalHelper(arr));
    }
    ...
    private static boolean partitionEqualSubsetSumOptimalHelper(int[] arr) {
        int sum = 0;
        for(int i = 0; i< arr.length; i++){
            sum += arr[i];
        }
        ...
        if(sum % 2 == 0){ // if the sum is even , then only we will go forward
            int target = sum/2;
            return subsetSumEqualsToTarget.subsetSumEqualsToTargetOptimal(arr, target); // this will return true or false on the basis of whether the arr has elements which sum up to target ( which is sum/2 )
        }else{
            return false; // if the sum is odd, then there is no way that an array can be devided into two subsets with equal sum
        }
    }
